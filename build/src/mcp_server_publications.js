...
import { CallToolRequestSchema as CallToolRequestSchema, ListToolsRequestSchema as ListToolsRequestSchema, ListResourcesRequestSchema as ListResourcesRequestSchema, ReadResourceRequestSchema, McpError, ErrorCode, } from "@modelcontextprotocol/sdk/types.js";
import { z } from "zod";
...
function toSummary(pub) {
    try {
        // Safely handle authors arrays
        const authors = (pub.authors || []).concat(pub.authors2 || []);
        const authorNames = authors
            .filter((a) => a && a.first_name && a.last_name) // Filter out invalid authors
...
async function searchPublicationsByAuthor(authorTerm) {
    try {
...
async function start_Mcp_server() {
    ...
    const server = new Server({
        ...
        capabilities: {
            resources: {
                publications: {
                    search: {
                        description: "Search for publications by author name and return a list with IDs, names, categories, title, and basic info",
                    },
                },
            },
            tools: {
                ...
                getPublicationById: {
                    description: "Get complete detailed information about a publication using its ID",
                },
            },
        },
    });
        ...
        switch (name) {
            case "searchPublicationsByAuthor": {
                ...
                try {
                    ...
                    const resultsText = publications.map((p, index) => `${index + 1}. Publication ID: ${p.id}
                        Title: ${p.title}
                        Author(s): ${p.author}
                        Category: ${p.category}
                        Year: ${p.year}`).join('\n\n');
                    ...
                    throw new McpError(ErrorCode.InternalError, `Error searching publications: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            case "getPublicationById": {
                ...
                try {
                    ...
                    const authorDetails = authors
                        .filter((a) => a && a.first_name && a.last_name)
                        .map((a) => {
                    ...
                    const projects = (publication.projects || [])
                        .map((proj) => `${proj.project_title || 'Untitled Project'} (ID: ${proj.id})`)
                        .join(', ');
                    // Format research tasks
                    const researchTasks = (publication.research_tasks || [])
                        .map((task) => task.title || task.name || 'Untitled Task')
                        .join(', ');
                    // Format tags
                    ...
                    AUTHORS:
                    ${authorDetails.length > 0 ? authorDetails.map((author, i) => `  ${i + 1}. ${author}`).join('\n') : '  No authors listed'}
                    ...
                    throw new McpError(ErrorCode.InternalError, `Error retrieving publication: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            default:
            ...
            resources: [
                {
                    ...
                    description: "Search for publications by author name to get their IDs",
                    mimeType: "text/plain",
                },
            ],
        };
